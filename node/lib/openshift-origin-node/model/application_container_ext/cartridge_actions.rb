      ...
      module CartridgeActions
        ...
        def create_public_endpoints(cart_name)
          ...
          cart.public_endpoints.each do |endpoint|
            ...
            output << "NOTIFY_ENDPOINT_CREATE: #{endpoint.public_port_name} #{ip_address} #{public_port} #{private_ip} #{endpoint.private_port}\n" 
        ...
        def update_cluster(cluster)
          ...
          cluster.split(' ').each do |line|
            ...
            registry_updates[uuid] = ::OpenShift::Runtime::GearRegistry::Entry.new(uuid: gear_uuid,
                                                                                   namespace: namespace,
                                                                                   dns: gear_dns,
